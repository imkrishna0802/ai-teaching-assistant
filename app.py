...
def _handle_quick_reply(label: str, message: str, key: str):
    if st.button(label, key=key):
        st.session_state.support_messages.append({"role": "user", "content": message})
        st.session_state.support_messages.append(
            {"role": "assistant", "content": get_support_response(message)}
        )
        st.rerun()
...
def render_support_chat():
    ...
    if st.session_state.support_open:
        ...
        with quick1:
            _handle_quick_reply("How to use", "How do I use this app?", "quick_use_btn")
            _handle_quick_reply("Fix FAISS", "FAISS is not working", "quick_faiss_btn")
        with quick2:
            _handle_quick_reply("API key help", "How to set Gemini API key?", "quick_api_btn")
            _handle_quick_reply("Import error", "ModuleNotFoundError", "quick_import_btn")
